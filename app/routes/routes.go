...
var customerValidControllers = &controllers.CustomerValidController{}       // 客户有效性标签控制器
...
func InitRoutes(engine *gin.Engine) {
	...
	api := engine.Group("/api")
	{
		...
		protected.Use(middleware.CasbinMiddleware())
		{
			...
			// 客户有效性标签路由组
			customerValid := protected.Group("/customerValid")
			{
				// 客户有效性标签列表
				customerValid.GET("/list", customerValidControllers.List)
				// 根据ID获取客户有效性标签信息
				customerValid.GET("/:id", customerValidControllers.Detail)
				// 新增客户有效性标签
				customerValid.POST("/add", customerValidControllers.Create)
				// 更新客户有效性标签
